...
const createInstrumento = async (req, res) =>{
    try {
        ...
        await instrumentoService.Create(name, category, description, price)
        res.sendStatus(201)
    } catch(error){
        console.log(error)
...
const updateInstrumento = async (req, res) => {
    try {
        if(ObjectId.isValid(req.params.id)){
            const id = req.params.id
            const { name, category, description, price} = req.body
            const instrumento = await instrumentoService.Update(id, name, category, description, price)
...
export default {getAllInstrumentos, createInstrumento, deleteInstrumento, updateInstrumento, getOneInstrumento}